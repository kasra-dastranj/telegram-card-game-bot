...
**تاریخ**: فوریه 2026  
**نسخه**: 2.2.0  
**وضعیت**: Production Ready — Updated with Corrected Rules + Mining & Arena Boost Fixes
...
وقتی هر دو انتخاب کردند:
  ...
  → مقایسه
  → اعلام برنده راوند
  → کاهش ۱-۲ واحد از ویژگی برنده
```

**فرمول کاهش**:
```python
def reduce_stat(stat_value, win_margin):
    if win_margin >= 5:
        reduction = 2
    else:
        reduction = 1
...
با این تفاوت که stats کاهش یافته از راوند قبل استفاده می‌شود
```
