"""Switch the staged Deck final-round reveal release on the TelBattle VPS."""
import hashlib
import json
import os
...
PREVIOUS = ROOT / 'releases/20260925-quick-sum-v1'
RELEASE = ROOT / 'releases/20260925-deck-final-reveal-v1'
...
def switch(target):
    temporary = ROOT / 'current-deck-final-reveal-20260925.tmp'
...
def main():
    ...
    manifest = json.loads(Path('/tmp/telbattle-deck-final-reveal-20260925-manifest.json').read_text())
    ...
    backup = ROOT / 'backups/game_bot-before-deck-final-reveal-20260925.db'
    if backup.exists():
