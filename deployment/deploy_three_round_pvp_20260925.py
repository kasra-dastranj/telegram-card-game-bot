"""Switch the staged three-round Mini App PvP release on the TelBattle VPS."""
import hashlib
import json
import os
...
PREVIOUS = ROOT / 'releases/20260925-management-lock-v1'
RELEASE = ROOT / 'releases/20260925-three-round-pvp-v1'
...
def switch(target):
    temporary = ROOT / 'current-three-round-pvp-20260925.tmp'
...
def verify_services():
    ...
    try:
        urllib.request.urlopen('http://127.0.0.1:5001/api/v1/three-round/matchmaking', timeout=10)
    except urllib.error.HTTPError as error:
        if error.code not in (401, 403, 405):
            raise RuntimeError('Three-round route unavailable: ' + str(error.code))


def main():
    if CURRENT.resolve() != PREVIOUS:
        raise RuntimeError('Live release changed; refusing to overwrite it')
    manifest = json.loads(Path('/tmp/telbattle-three-round-pvp-20260925-manifest.json').read_text())
    ...
    backup = ROOT / 'backups/game_bot-before-three-round-pvp-20260925.db'
    if backup.exists():
