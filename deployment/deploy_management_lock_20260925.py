"""Switch the staged Mini App management lock release on the TelBattle VPS."""
import hashlib
import json
import os
...
PREVIOUS = ROOT / 'releases/20260925-deck-form-clarity-v1'
RELEASE = ROOT / 'releases/20260925-management-lock-v1'
...
def switch(target):
    temporary = ROOT / 'current-management-lock-20260925.tmp'
...
def main():
    ...
    manifest = json.loads(Path('/tmp/telbattle-management-lock-20260925-manifest.json').read_text())
    ...
    backup = ROOT / 'backups/game_bot-before-management-lock-20260925.db'
    if backup.exists():
